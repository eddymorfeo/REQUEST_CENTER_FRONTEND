...
type TerminalFilter = "all" | "terminal" | "not_terminal";
...
export default function StatusListView() {
  ...
  const [terminalFilter, setTerminalFilter] = React.useState<TerminalFilter>("all");
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      setError(getErrorMessage(error) || "Error cargando estados.");
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const filteredRows = React.useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      const matchesTerminal =
        terminalFilter === "all" ||
        (terminalFilter === "terminal" && row.is_terminal) ||
        (terminalFilter === "not_terminal" && !row.is_terminal);

      return matchesSearch && matchesStatus && matchesTerminal;
    });
  }, [rows, search, statusFilter, terminalFilter]);

  const activeStatuses = rows.filter((row) => row.is_active).length;
  const hasFilters = search.trim() !== "" || statusFilter !== "all" || terminalFilter !== "all";
    ...
    >
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="space-y-1">
          <h1 className="text-2xl font-semibold tracking-tight">Gestion de Estados</h1>
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,360px)_minmax(0,360px)]">
        <div className="rounded-xl border bg-background p-5 shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="text-sm text-muted-foreground">Estados totales</div>
        ...
        <div className="rounded-xl border bg-background p-5 shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="mt-1 text-3xl font-semibold leading-none text-emerald-700">{activeStatuses}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl border bg-background p-5 shadow-sm">
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-[360px_360px_1fr_auto] lg:items-end">
          ...
          <div className="space-y-2">
            ...
            <Select value={terminalFilter} onValueChange={(value) => setTerminalFilter(value as TerminalFilter)}>
              <SelectTrigger className="h-10 rounded-lg">
                <SelectValue />
              </SelectTrigger>
              <SelectContent position="popper" align="start" className="w-[var(--radix-select-trigger-width)]">
                ...
                <SelectItem value="not_terminal">No terminales</SelectItem>
              </SelectContent>
            </Select>
          </div>
            ...
            {isLoading ? (
              ...
              pageRows.map((row) => (
                <TableRow key={row.id}>
                  ...
                  <TableCell>{row.is_terminal ? "Si" : "No"}</TableCell>
                  <TableCell>
