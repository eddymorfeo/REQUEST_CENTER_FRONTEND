...
type Props = {
  ...
  onRequestsChange: React.Dispatch<React.SetStateAction<RequestItem[]>>;
};
...
export function RequestsBoardView({
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    const targetStatusId = event.over?.id ? String(event.over.id) : null;
    ...
    const targetStatusName =
      statuses.find((s) => s.id === targetStatusId)?.name ?? "nuevo estado";

    // ✅ Confirmación (necesita alerts.confirm)
    ...
    onRequestsChange((prev) =>
      prev.map((r) => (r.id === requestId ? { ...r, status_id: targetStatusId } : r))
    );
    ...
    try {
      ...
      // ✅ rollback
      onRequestsChange((prev) =>
        prev.map((r) => (r.id === requestId ? { ...r, status_id: prevStatusId } : r))
      );

      await alerts.error("No se pudo actualizar el estado", e?.message ?? "Intenta nuevamente.");
    }
  };
